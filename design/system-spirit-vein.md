...
> 📌 **奖励落地说明：**
> - "解锁高级丹方"：命中时向驻守的所有玩家写入 `character_unlocked_recipes`（随机选一个该玩家未解锁的高级丹方；全已解锁则改发等价灵石）
> - "觉醒石/附灵石"：命中时按 60/40 概率选一个，写入 `character_materials`（复用现有材料掉落通道）
...
| 收益方 | 分配规则 |
|-------|---------|
| 宗门整体 | 节点产出基数 × 60% → 宗门资金 / 宗门仓库 |
| 守卫本人 | 节点产出基数 × 30% → 平分给当时在守的玩家（灵石入背包、修为直接加身） |
| 贡献度 | 每次涌灵给守卫玩家 +100 贡献度 |
| 奖池留存 | 10% → 全服灵脉大奖池（见 §5.3） |

**极品 / 上品节点额外掉落：** 按 §2.1 概率掉落稀有奖励，直接进入**当次参与涌灵的守卫玩家**个人背包 / 丹方解锁记录（不进宗门仓库，避免分配争议）。
...
| 进攻方上限 | 单次偷袭最多组队 **= 守卫上限**（下品 2 人、中品 3 人等） |
| 组队限制 | 必须全部来自**同一宗门** |
...
| 保护内容 | 保护期内**所在宗门成员**不可被偷袭（若守卫在保护期宗门，整个节点免疫进攻） |
| 放弃保护 | 宗主可主动放弃保护期（通常为了占极品节点） |
...
**仇视机制：** 宗门一旦占领 ≥ 4 个节点，24 小时后自动成为"**众矢之敌**"：
- 对其偷袭的进场费 × 0.5
- 偷袭胜方额外获得 +500 贡献度
...
| 排名 | 奖励来源 | 分配 |
|------|---------|------|
| 本周总涌灵次数 TOP1 宗门 | 奖池 40% | 进宗门仓库 |
| TOP2-3 宗门 | 奖池 20% (各 10%) | 进宗门仓库 |
| 全服参与偷袭 ≥ 20 次玩家 | 奖池 30% | 平分进个人背包 |
| 奖池留存 | 10% | 滚入下周奖池 |
...
| 频率 | 任务 | 说明 |
|------|------|------|
| 每 2 小时 (偶数整点) | 涌灵结算 | 遍历 6 节点，计算收益 → 宗门 + 守卫 |
| 每 5 分钟 | 真空期扫描 | 真空期结束且无人驻守的节点 → NPC 接管 |
| 每 10 分钟 | 驻守过期扫描 | guard.expires_at 到期 → 自动离岗 |
| 每日 00:00 | 每日偷袭计数重置 | 按日期分区存储，到期清空 |
| 每周日 23:55 | 奖池结算 | 根据涌灵排行 / 偷袭次数分发奖池 |
...
- 涌灵结算后，守卫玩家 Toast："灵脉涌动 — 获得 X 灵石 / Y 修为 / Z 贡献"
- 被偷袭失败时，前守卫玩家离线消息："你驻守的 {节点名} 被 {宗门名} 偷袭得手"
...
### 前置依赖
> ⚠️ 本系统**依赖宗门战**已先行完成以下两项基础设施，开工前请确认：
> 1. `timed_buffs` 通用表已建立（用于未来可能的节点 Buff 扩展）
> 2. N vs N 多人战斗引擎已可用（复用于偷袭战）
...
6. 涌灵奖励发放器 — 灵石/修为入账 + 稀有掉落（解锁丹方走 `character_unlocked_recipes`；觉醒石/附灵石走 `character_materials`）
7. 战报持久化到 `spirit_vein_raid.battle_log` (JSONB)
...
| 灵脉 Buff 不叠加宗门战加成 | 两套加成独立计算，避免数值飞升 |
...
| 是否做"节点被动光环"给周边玩家 | 否 | v2 可加"极品节点附近玩家 +5% 修为" |
