...
| 会心率 `critRate` | 12% | 30%（clamp） | 引擎 50% cap |
| 会伤 `critDmg` | 60% | 不叠加上限（但概率低） | 引擎 300% cap |
...
**设计原则**：装备附灵是**倾向性增益**，不是数值核心。与境界、装备主副属性叠加后，玩家的"换装爽感"来自**新触发效果解锁**（"我居然会心反伤了！"），而不是"数值翻倍"。
...
| 槽位 | 名称 | 定位 | 效果方向 | 本期条数 |
|------|------|------|---------|--------|
| `weapon` | 兵器 | 输出核心 | 进攻性（吸血 / 持续伤害 / 会心 / 斩杀 / 元素） | **13** |
| `armor` | 法袍 | 生存核心 | 防御性（反伤 / 减伤 / 回血 / 抗性 / 免控） | **13** |
| `pendant` | 灵佩 | 辅助增益 | 功能性（身法 / 会伤 / 神识 / 掉落 / 资源） | **12** |
...
| `aw_keen` | 锋锐 | 会心率 +X% | 复用 `critRate` | 3 | 5 | 8 | 12 |
...
| `aw_crit_bonus` | 噬心 | 会心伤害 +X% | 复用 `critDmg` | 10 | 18 | 28 | 40 |

### 3.2 法袍附灵池（14 条 / 生存向，v3.6 +1）

| ID | 名称 | 效果 | 实现 | 蓝 | 紫 | 金 | 红 |
|----|------|------|-----|----|----|----|----|
| `aw_adamant` | 金刚 | 防御% +X% | 主属性防御% 加成 | 6 | 10 | 15 | 22 |
| `aw_bastion` | 磐石 | 气血% +X% | 主属性气血% 加成 | 6 | 10 | 15 | 22 |
| `aw_thorns` | 荆棘 | 受会心时 X% 反弹伤害 | 复用 `reflectOnCrit` | 15 | 25 | 35 | 50 |
| `aw_mirror_armor` **v3.6** | 明镜甲 | 受击时常驻反弹 X% 伤害 | 新增 `reflectPct` 走 equipReflectPct 通道 | 6 | 10 | 15 | 22 |
...
| `aw_crit_shield` | 金钟罩 | 受到会心时伤害再减 X% | 新增 `critTakenReduction` | 10 | 18 | 25 | 35 |

### 3.3 灵佩附灵池（13 条 / 辅助向，v3.6 +1）

| ID | 名称 | 效果 | 实现 | 蓝 | 紫 | 金 | 红 |
|----|------|------|-----|----|----|----|----|
| `aw_swift` | 疾风 | 身法% +X% | 主属性身法% 加成 | 6 | 10 | 15 | 22 |
| `aw_mirror_pendant` **v3.6** | 玄镜佩 | 受击时常驻反弹 X% 伤害 | 新增 `reflectPct` 走 equipReflectPct 通道 | 5 | 8 | 12 | 18 |
| `aw_doom` | 玄冥 | 会伤 +X% | 复用 `critDmg` | 15 | 25 | 40 | 60 |
...
**分配规则**（v2026-05-14：组队秘境奖励统一改为纯随机）：
- 附灵石：按**人头均分**，余数随机分给一位队员
- 灵枢玉：**每件均匀随机派发**给某位队员
...
| `critTakenReduction` | 玩家受会心结算 | `if (isCrit) incoming *= 1 - p.critTakenReduction` |
...
- [ ] 附灵石按人头均分分配；灵枢玉每件均匀随机派发（v2026-05-14 改）
...
| 会心率 | 锋锐 12% + 境界 6% + 副属性最多 ~15% | — | ~33% | ✅ 未破 60% 玩家 cap |
| 会伤 | 噬心 40% + 玄冥 60% | 境界 30% + 副属性 ~36% | 166% | ✅ 未破 300% 引擎 cap |
