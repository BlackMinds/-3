...
3. **奖励纯随机派发** —— 物品类奖励均匀随机分配；经验/积分每人全额（v2026-05-14 改）
4. **保底机制** —— 装备每人至少 1 件保底，即使运气差也不白跑
...
|----------|------|---------|
| 灵石（怪物掉落） | — | 已停发 |
| 经验 | 修炼经验 + 等级经验 | 每人独立全额（不分摊） |
| 秘境积分 | 组队专属货币 | 每人独立全额（不分摊） |
| **高品质装备** | 绿~红装备（品质权重向高阶倾斜） | 每人保底 1 件，剩余每件均匀随机派发 |
| **Boss 保底装备** | 难度越高保底品质越高（紫/金/红） | 与普通装备同池，纯随机 |
| 秘境灵草 | 稀有灵草（分品质） | 均分 |
| 附灵石 / 强化石 | 附灵 / 强化材料 | 均分（余数随机一人） |
| 灵枢玉 | 附灵重随材料 | 每件均匀随机派发 |
| 首通奖励 | 秘境积分/宝箱/称号（详见 5.7） | 全队每人首通获得 |

> v2026-05-14：秘境**不再掉落功法残页**（功法获取仍可走单人刷图、通天塔、宗门商店、万能残页等渠道）。

> v2026-05-14 改动：贡献度仅作战报展示，**不再影响任何奖励分配**。

### 5.3 灵石奖励

怪物掉落灵石已停发；秘境的物质收益主要体现在装备掉落与秘境积分商店兑换。
...
v2026-05-14 起改为**纯随机派发**：

```
1. 每人保底 1 件：从全场掉落池中随机抽 1 件给每位队员
2. 剩余装备：每件均匀随机派发给某位队员（红/金装与普通装备同池）
3. 贡献度仅作战报展示，不影响任何分配
```

> 设计原因：贡献度本就受装备/境界影响，再以贡献分配奖励容易造成强者愈强；改为纯随机让组队体验更轻松。
...
> v2026-05-14：贡献度**不再参与奖励分配**，仅作战报展示用途（每人造成的伤害/治疗/承伤的归一化指标）。
...
三个维度覆盖三种职责（DPS / 治疗 / 坦克），战报里能看到每位玩家的多维数据。
...
> 该值仅写入 `secret_realm_contributions.contribution` 字段供战报和历史记录展示，**不参与任何分配权重**。
...
> 战斗是 AI 全自动的，奖励为纯随机派发，无须额外反挂机机制。
...
| 奖励直接入库 | 奖励纯随机派发后入库（v2026-05-14：贡献仅作战报展示，不再参与分配） |
...
| 蹭车玩家贡献极低 | 其他队员奖励被稀释（纯随机制下风险弱化） | v2026-05-14 起奖励纯随机派发，蹭车不会"拿走高战利品"；房间防刷由准备超时 + 创房限频处理 |
