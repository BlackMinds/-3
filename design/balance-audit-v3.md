...
| `server/engine/battleEngine.ts:447-448` | 会心:random < crit_rate 则 damage ×= crit_dmg |
...
**好词条系数最低**：避开引擎 cap（会心 50% / 会伤 320% / 吸血 15% / 闪避 30%）。

#### 2. CRIT_DMG 削弱（`server/utils/equipment.ts`）

`min: 2 → 1, max: 8 → 6`（v3.4 已经从 10→8，本次再 -25%，min 再 -50%）

理由：会伤是引擎中唯一无 cap 软压制的好词条（cap 320% 极难撞到），随 tier 浮动后单条期望太高，需要把 base 范围下调以保持稀缺性。
...
2. **怪物配套不动**：玩家小幅上升的副属性数值不会撞引擎 cap（会心 80% / 会伤 320% / 吸血 30% / 闪避 40%）。
