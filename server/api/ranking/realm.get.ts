...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      WHERE c.name <> '吴彦祖1号'
      ORDER BY c.realm_tier DESC, c.realm_stage DESC, c.cultivation_exp DESC
      LIMIT 50
    `)
    ...
    const { rows: charRows } = await pool.query(
      'SELECT id, realm_tier, realm_stage, cultivation_exp FROM characters WHERE user_id = $1', [event.context.userId]
    )
    let myRank = null
    if (charRows.length > 0) {
      ...
      const { rows: countRows } = await pool.query(`
        ...
        WHERE name <> '吴彦祖1号' AND (
              realm_tier > $1 OR (realm_tier = $2 AND realm_stage > $3)
